...
- Автотесты (`backend/tests/test_engine.py`, 7 тестов) — в том числе формальная
  проверка ровно того примера-ловушки, который описан в ТЗ.
...
3. Отметить любой рекомендованный шаг выполненным (кнопка на странице или
   ...
   разрыв уменьшается, готовность растёт.
...
- Локализация интерфейса (kk/ru/en) реализована для всех страниц (`backend/app/i18n.py`).
  Язык страницы сотрудника берётся из его `preferred_language` — в README датасета это
  поле прямо описано как "UI language preference", так что это не декоративная фича, а
  честная связь с данными; можно переопределить через `?lang=` (для проверки). На
  странице входа и HR-панели — переключатель RU/KK/EN, выбор сохраняется в cookie.
